...
def read_excel_df(excel_path):
    ...
    excel_df['仕入れ日'] = pd.to_datetime(excel_df['仕入れ日'], format="%Y/%m/%d").astype(str)
    excel_df['id'] = excel_df['仕入れ日'].str.cat(excel_df['ASIN'])
    return excel_df


def read_sedolist_csv(sedori_path: str):
    sedolist_df = pd.read_csv(sedori_path, encoding="cp932")
    ...
    format_df['仕入れ日'] = pd.to_datetime(format_df['仕入れ日'], format="%Y/%m/%d").astype(str)
...
def to_write(sedori_path: str, excel_path=excel_path, output_path=None):
    excel_df = read_excel_df(excel_path)
